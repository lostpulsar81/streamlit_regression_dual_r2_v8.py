...
def safe_output_stem(sheet_name):
    cleaned = re.sub(r"[^A-Za-z0-9._-]+", "_", str(sheet_name).strip())
...
with left_col:
    ...
    if sheet_name:
        st.caption(f"Output file name: {safe_output_stem(sheet_name)}.{save_format}")
...
with right_col:
    ...
    if uploaded_file and sheet_name:
        try:
            ...
            output_stem = safe_output_stem(sheet_name)
            img_buffer = io.BytesIO()
            savefig_kwargs = dict(format=save_format, dpi=300, bbox_inches="tight")
            if save_format == "png":
                savefig_kwargs["transparent"] = True
            fig.savefig(img_buffer, **savefig_kwargs)
            img_buffer.seek(0)
            ...
            with download_col:
                st.download_button(
                    ...
                    data=img_buffer,
                    file_name=f"{output_stem}.{save_format}",
            ...
            with res_col2:
                with st.container(border=True):
                    ...
                    st.write("- Downloaded **PNG** files use a transparent background.")
